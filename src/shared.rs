...
where
    E: FnMut(&[f64], &mut [f64]) -> Result<f64>,
{
    pub(crate) problem: &'a mut LbfgsbProblem<E>,

    pub(crate) param: LbfgsbParameter,
    ...
    pub(crate) iwa: Vec<i64>,

    // csave is a working string of characters of length 60.
    // static char csave[60];
    pub(crate) csave: [i64; 60],
...
where
    E: FnMut(&[f64], &mut [f64]) -> Result<f64>,
{
  pub(crate) fn new(problem: &'a mut LbfgsbProblem<E>, param: LbfgsbParameter) -> Self {
    let n = problem.x.len();
    let m = param.m;
